# Full Architecture Blueprint & Database Schema (Full Source)

Here is the **complete PostgreSQL schema**, organized by role/domain, in the order a real implementation would build it. Every table includes multi-tenant fields (`tenant_id`, `hospital_id`), audit columns, and soft-delete support unless noted. I've included the newly identified modules (doctor payouts, TPA rate contracts, discount approvals, consent artefacts, blood bank, shift/leave, data processors) as well.

---

## 0. Common Conventions (applies to every table below)

```sql
CREATE EXTENSION IF NOT EXISTS "pgcrypto";
CREATE EXTENSION IF NOT EXISTS "pg_trgm"; -- fuzzy search on names/mobile

-- Standard audit columns added to almost every table:
...
## 1. SUPER ADMIN DOMAIN (Platform level — no tenant_id, this owns tenants)
...
CREATE TABLE tenants (
    ...
    db_connection_ref VARCHAR(200), -- only used if dedicated_db
...
CREATE TABLE subscription_plans (
    ...
    max_users INT,
    max_beds INT,
    modules JSONB NOT NULL, -- {"ipd":true,"ot":true,"pharmacy":true,"pacs":false,"abdm":true}
...
CREATE TABLE global_masters_drug (
    ...
    schedule_class VARCHAR(10), -- H, H1, X, etc. (CDSCO)
...
CREATE TABLE departments (
    ...
    department_type VARCHAR(50), -- OPD, IPD, Diagnostic, Support
    is_active BOOLEAN DEFAULT true
);
...
## 3. IDENTITY, RBAC, HR & PAYROLL (Hospital Admin domain)
...
CREATE TABLE users (
    ...
    nmc_registration_no VARCHAR(50), -- for doctors, required for telemedicine
...
CREATE TABLE permissions (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    permission_key VARCHAR(100) UNIQUE NOT NULL, -- e.g. billing.discount.approve
    module VARCHAR(50),
    description TEXT
);
...
-- Doctor revenue-share / payout engine
CREATE TABLE doctor_payout_rules (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id UUID NOT NULL,
    hospital_id UUID NOT NULL,
    doctor_id UUID NOT NULL REFERENCES users(id),
    service_category VARCHAR(50), -- OPD, IPD, Surgery, Procedure
    payout_type VARCHAR(20) CHECK (payout_type IN ('percentage','flat','tiered')),
    payout_value NUMERIC(10,2), -- % or flat amount
...
CREATE TABLE patients (
    ...
    aadhaar_masked VARCHAR(20), -- store only last 4 digits
    marital_status VARCHAR(20),
    occupation VARCHAR(100), -- optional field, governed by data_collection_policy
    photo_url TEXT,
...
## 9. LABORATORY / RADIOLOGY (LIS / RIS / PACS)
...
## 11. ABDM / FHIR / CONSENT / COMPLIANCE
...
## 12. ROW-LEVEL SECURITY (applied to every tenant table)
...
| Role | Primary Tables Owned |
|---|---|
| Super Admin | tenants, subscription_plans, subscriptions, tenant_feature_flags, global_masters_* |
| Hospital Admin | hospitals, hospital_settings, departments, wards, beds, roles, permissions, service_tariffs, discount_rules, print_templates |
| Reception | patients, patient_identifiers, appointments, queue_tokens, notification_logs, patient_insurance_details |
| Doctor | encounters, clinical_notes, diagnoses, prescriptions, prescription_items, referrals, teleconsultation_sessions, admissions (initiate) |
| Nurse | vitals, nursing_notes, medication_administration_records, diet_plans, shift_handovers, bed_transfers |
| Pharmacy | inventory_items, stock_batches, stock_transactions, dispenses, controlled_drug_register, purchase_orders |
| Lab | lab_orders, lab_order_items, lab_results, critical_result_escalations |
| Radiology | radiology_orders, radiology_reports |
| Billing/TPA | invoices, invoice_lines, payments, payment_refunds, insurance_claims, claim_documents, claim_settlements, discount_approvals, rate_contracts |
| HR | users, staff_shifts, shift_rosters, leave_types, leave_balances, leave_requests, attendance_records, payroll_runs, payslips |
| OT Staff | ot_bookings, ot_checklists, anesthesia_notes, post_op_notes, surgical_implants_consumables |
| Blood Bank | blood_donors, blood_units, blood_cross_matches |
| Compliance/DPO | consents, consent_artefacts, data_processors, data_processor_access_logs, audit_logs, medication_errors, adverse_events |
| Patient (self-service) | reads own patients/invoices/prescriptions/lab_results; writes to consents |
