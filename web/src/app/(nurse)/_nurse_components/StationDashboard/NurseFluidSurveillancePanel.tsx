...
export const NurseFluidSurveillancePanel: React.FC = () => {
  ...
  const positiveCount = fluidLogs.filter((f) => f.net > 0).length;
  const negativeCount = fluidLogs.filter((f) => f.net < 0).length;
  const criticalCount = fluidLogs.filter((f) => f.status === "CRITICAL_POSITIVE").length;
    ...
    <div className="bg-white p-4 sm:p-5 rounded-xl border border-slate-200 shadow-xs space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="p-3 rounded-lg bg-blue-50 border border-blue-200 text-center">
          <span className="text-3xs font-bold text-blue-800 uppercase">Positive Net Balance</span>
          <h4 className="text-xl font-bold text-blue-900 mt-0.5">{positiveCount} Patients (+mL)</h4>
        </div>
        <div className="p-3 rounded-lg bg-purple-50 border border-purple-200 text-center">
          <span className="text-3xs font-bold text-purple-800 uppercase">Negative Net Balance</span>
        ...
        <div className="p-3 rounded-lg bg-rose-50 border border-rose-200 text-center">
          <span className="text-3xs font-bold text-rose-800 uppercase">Critical Fluid Overload</span>
            ...
            >
              <div>
                <div className="flex items-center gap-1.5 font-bold text-slate-900">
                  <span className="font-mono text-3xs bg-teal-100 text-teal-800 px-1.5 py-0.5 rounded">{fl.bedNumber}</span>
                  {fl.patientName}
                </div>
              ...
              <div className="text-right">
                ...
                {fl.status === "CRITICAL_POSITIVE" && (
                  <Tag color="error" className="text-3xs font-bold font-mono mt-0.5">REVIEW REQUIRED</Tag>
                )}
              </div>
            </div>
          ))}
