    ...
    <div className="bg-white p-4 sm:p-5 rounded-xl border border-slate-200 shadow-xs space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="p-3 rounded-lg bg-emerald-50 border border-emerald-200 text-center">
          <span className="text-3xs font-bold text-emerald-800 uppercase">Accepted Handovers</span>
          <h4 className="text-xl font-bold text-emerald-900 mt-0.5">{acceptedCount} Handovers</h4>
        </div>
        <div className="p-3 rounded-lg bg-amber-50 border border-amber-200 text-center">
          <span className="text-3xs font-bold text-amber-800 uppercase">Pending Review</span>
            ...
            >
              <div>
                <div className="flex items-center gap-1.5 font-bold text-slate-900">
                  <span className="font-mono text-3xs bg-teal-100 text-teal-800 px-1.5 py-0.5 rounded">{ho.bedNumber}</span>
                  {ho.patientName} &bull; <span className="font-mono text-slate-500 text-3xs">{ho.shift}</span>
                </div>
                <p className="text-3xs font-mono text-slate-500 mt-0.5">
                  Outgoing: {ho.outgoing} &rarr; Incoming: {ho.incoming} &bull; {ho.notes}
                </p>
              </div>
              <Tag color={ho.status === "ACCEPTED" ? "emerald" : "warning"} className="text-3xs font-bold font-mono">
                {ho.status}
              </Tag>
            </div>
          ))}
