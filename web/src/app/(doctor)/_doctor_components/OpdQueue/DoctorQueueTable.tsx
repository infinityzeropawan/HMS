...
const INITIAL_QUEUE: QueuePatient[] = [
  { key: "1", tokenNo: "T-01", uhid: "P-2026-1049", name: "Sunil Verma",    ageGender: "45 / M", vitals: "BP:130/85 | Temp:98.6°F | O₂:98%", status: "WAITING",    waitMins: 24 },
  { key: "2", tokenNo: "T-02", uhid: "P-2026-1052", name: "Anjali Gupta",   ageGender: "32 / F", vitals: "BP:118/75 | Temp:99.1°F | O₂:99%", status: "WAITING",    waitMins: 18 },
  { key: "3", tokenNo: "T-03", uhid: "P-2026-1058", name: "Ramesh Kumar",   ageGender: "58 / M", vitals: "BP:145/90 | Temp:98.4°F | O₂:97%", status: "CALLING",    waitMins: 5  },
  { key: "4", tokenNo: "T-04", uhid: "P-2026-1063", name: "Priya Mehta",    ageGender: "27 / F", vitals: "BP:110/70 | Temp:98.2°F | O₂:100%",status: "WAITING",   waitMins: 35 },
  { key: "5", tokenNo: "T-05", uhid: "P-2026-1071", name: "Dinesh Bhatia",  ageGender: "61 / M", vitals: "BP:158/95 | Temp:99.4°F | O₂:95%", status: "WAITING",   waitMins: 41 },
];
...
export const DoctorQueueTable: React.FC<DoctorQueueTableProps> = ({ onQueueChange }) => {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("hms_doctor_queue");
      if (saved) {
        try {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      localStorage.setItem("hms_doctor_queue", JSON.stringify(queue));
    }
  ...
  const handleNoShow = (record: QueuePatient) => {
    updateStatus(record.key, "NO_SHOW");
    ...
    },
    {
      ...
      render: (m: number, r: QueuePatient) =>
        r.status === "WAITING" || r.status === "CALLING"
          ? <span className={`text-xs font-medium ${m > 30 ? "text-rose-600" : "text-amber-600"}`}><Clock className="w-3 h-3 inline mr-0.5" />{m}m</span>
    ...
    <div className="space-y-4">
      ...
      <div className="block sm:hidden space-y-3">
        {activeRows.map(patient => (
          <div key={patient.key} className={`p-4 rounded-xl border shadow-sm space-y-2 ${patient.status === 'CALLING' ? 'bg-blue-50/80 border-blue-200' : patient.status === 'CONSULTING' ? 'bg-emerald-50/80 border-emerald-200' : 'bg-white border-slate-200'}`}>
